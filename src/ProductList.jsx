...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (plant) => {
    if (addedItems.has(plant.name)) return;
    ...
    setAddedItems(new Set([...addedItems, plant.name]));
                  ...
                  <div key={`${plant.name}-${index}`} className="plant-card">
                    ...
                    <div className="plant-details">
                      ...
                      <button
                        ...
                        disabled={addedItems.has(plant.name)}
                        style={{
                          backgroundColor: addedItems.has(plant.name)
                            ? "#aaa"
                            : "#4CAF50",
                          color: "#fff",
                          padding: "10px 20px",
                          ...
                          fontSize: "16px",
                          fontWeight: "bold",
                          cursor: addedItems.has(plant.name)
                            ? "not-allowed"
                            : "pointer",
                          boxShadow: addedItems.has(plant.name)
                            ? "none"
                            : "0 4px 6px rgba(0, 0, 0, 0.1)",
                          transition:
                            "background-color 0.3s ease, transform 0.2s ease",
                          transform: addedItems.has(plant.name)
                            ? "none"
                            : "scale(1.02)",
                        }}
                      >
                        {addedItems.has(plant.name)
                          ? "✔ Added"
                          : "🛒 Add to Cart"}
                      </button>
                    </div>
